...
**Document structure**:
- `messages` collection: arp-compatible fields (`messageId`, `conversationId`, `user`, `parentMessageId`, `sender`, `text`, `content`, `tokenCount`, `inputTokenCount`, `finish_reason`, `endpoint`, `model`, `isCreatedByUser`) plus a `agentMessage` field containing the full pi `AgentMessage` object for context reconstruction. Token accounting is strictly three-tier: `tokenCount` = the message's own text size estimate (note: arp/LibreChat may overwrite `tokenCount`/`inputTokenCount` afterwards with its own full-context estimates — pi's canonical values are the fields below and `agentMessage`); `inputTokens`/`outputTokens`/`cacheReadTokens`/`cacheWriteTokens` = per-model-call usage of the (latest) call behind the assistant document; `totalInputTokens`/`totalOutputTokens`/`totalCacheReadTokens`/`totalCacheWriteTokens` on the turn's assistant document = turn-cumulative usage over all calls of the turn.
- `conversations` collection: arp-compatible fields (`conversationId`, `user`, `messages`, `endpoint`, `endpointType`, `agent_id`, `model`, `title`, `tags`, `files`, `finish_reason`) plus a `cwd` field for the working directory and session-cumulative `totalInputTokens`/`totalOutputTokens`/`totalCacheReadTokens`/`totalCacheWriteTokens` usage totals.
