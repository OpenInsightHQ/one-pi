...
export interface BashToolOptions {
	...
	/**
	 * Enable sandbox to restrict file access to cwd only.
	 * Also enabled globally when PI_BASH_SANDBOX=1 env var is set.
	 * Default: false
	 */
	sandbox?: boolean;
	/**
	 * Root directory the sandbox allows file access to (paths within this tree
	 * are readable/writable). Defaults to `cwd`. `cd` is still restricted to the
	 * session `cwd` so script outputs stay there. Set broader than cwd (e.g. a
	 * user's session directory) so the agent can query the user's files and run
...
export function createBashToolDefinition(
	...
	return {
		...
		async execute(
			...
			const sandboxEnabled = options?.sandbox || process.env.PI_BASH_SANDBOX === "1";
			if (sandboxEnabled) {
