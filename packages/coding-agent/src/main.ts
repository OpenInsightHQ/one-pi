...
function buildSessionOptions(
	...
	if (parsed.model) {
		...
		if (resolved.warning && !suppressModelWarning) {
...
export async function main(args: string[]) {
	...
	const shouldTakeOverStdout =
		(firstPass.mode !== undefined && firstPass.mode !== "http") || firstPass.print || !process.stdin.isTTY;
	...
	const hasSavedHttpModelConfig = mode === "http" && loadHttpModelConfig() !== undefined;
	const { options: sessionOptions, cliThinkingFromModel } = buildSessionOptions(
		...
		modelRegistry,
		settingsManager,
		hasSavedHttpModelConfig,
	);
	sessionOptions.authStorage = authStorage;
	sessionOptions.modelRegistry = modelRegistry;
	sessionOptions.resourceLoader = resourceLoader;
	sessionOptions.continueSession = parsed.continue;
	...
	if (!isInteractive && !session.model && mode !== "http") {
	...
	if (mode === "rpc") {
		...
		const port = parsed.httpPort ?? 3000;
		const savedModelConfig = loadHttpModelConfig();
		if (savedModelConfig) {
			console.log(
				chalk.dim(`Provider: ${savedModelConfig.provider}, Model: ${savedModelConfig.model} (from saved config)`),
			);
		} else if (session.model) {
			console.log(chalk.dim(`Provider: ${session.model.provider}, Model: ${session.model.id}`));
		}
		console.log(chalk.dim(`Starting HTTP API server on port ${port}...`));
		try {
			await startHttpServer(session, resourceLoader, { port });
		} catch (err) {
			console.error(chalk.red(`HTTP server error: ${err}`));
			throw err;
		}
		stopThemeWatcher();
		restoreStdout();
