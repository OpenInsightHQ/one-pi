...
- Standalone credential references. `skills`/`mcpservers` documents gain `credentialRef` naming a standalone credential (`credentials` docs with `resourceType: "credential"`, created in dmp's credential management). `resolveCredentialsWithRef`/`hasCredentialsWithRef` resolve the referenced credential with strict either/or semantics — admin-configured values win (admin-managed), otherwise only the user's own binding of that credential counts (no fallback chain). Declaration-only docs (`data == null` — schema declared, values pending) are skipped by resolution and existence checks. Used by the catalog status markers and all three `skill_execute` executors (http/mcp/script). `buildCredentialHeaders` falls back to using credential field names (`secretKey`) as header keys when `authType: "headers"` has no explicit `headerMap`. `CredentialResourceType` and the mongoose enum accept `"credential"`; `SkillCredentialDoc` gains a plaintext `schemaJson` field (bind-form rendering for standalone entries, values stay encrypted).
- Personal skill sync and outer-agent catalog alignment (§5.6/§5.7 of the skill credential plan). `personal-skill-sync.ts` lazily upserts skill-creator/upload output from the user's personal skills directory into the `skills` collection (idempotent on `(name, author)`, `source: "skill-creator"`, `userManaged: true`; `requiresSecrets`/`requires-secrets` SKILL.md frontmatter parsed into `credentialSchema`), soft-disables records whose directory disappeared, and is throttled to one scan per user per 60s. Triggered from `createHttpResourceLoader` (session catalog assembly), `POST /skills/sync` (`force` bypasses the throttle), and `POST /skills/register-personal` (single-skill best-effort registration for the skill-creator instructions). New `GET /skills/catalog` returns the rendered catalog blocks plus machine-readable http/mcp entries — the single source of truth arp composes its outer agent prompt from. `POST /execute-agent-skill` now routes http/mcp skill names to a `skill_execute` instruction (repo skills keep the `/skill:` subagent expansion), so arp's `execute_skill` tool works unchanged across all three skill kinds.
- Two-stage skill catalog and unified dispatch tools (M3 of the skill credential system). `catalog-service.ts` renders O(N) `<available_http_skills>` / `<available_mcp_skills>` blocks appended per turn to the system prompt of `/prompt`, `/v1/chat/completions` (hidden in skill-execution mode, like the repo catalog). http skills are read directly from the `skills` collection via inline `apiDefinitions` — no install/symlink to the pi environment (`savePath` no longer required for `skillType: "http"`); MCP servers are read from the `mcpservers` collection (own + ACL `resourceType: "mcp"`). Entries carry a credential status marker so the model can guide users to un-bound skills. New agent tools `skill_describe` (list one skill's APIs/tools — http from `apiDefinitions` filtered by `exposeToModel`, mcp via read-only `probeMcpServerTools` with a 5-min per-user cache) and `skill_execute` (single choke point: http fetch with credential headers + param validation, mcp call via exported `callMCPTool` with credential-bound headers, script skills via server-side `python scripts/main.py` spawn — no shell, params over stdin, credentials only in the child env, output masked with exact-value scrubbing). Missing credentials return a structured guidance telling the model to direct the user to the portal instead of collecting secrets in-chat. Registered in `/prompt`, `/v1/chat/completions`, and `/upload` file-processing sessions.
- Skill/MCP credential store (M1 of the cross-platform skill credential system, see `docs/credential-skill-dev-plan.md`). New `credentials` MongoDB collection stores per-principal encrypted credential bindings (AES-256-GCM, master key from `PI_CREDENTIAL_MASTER_KEY` env, base64 32-byte; the same cipher spec is implemented by dmp/Java and arp/Node). New `credential-service.ts`: `setCredentials`/`deleteCredentials` (encrypted upsert, never plaintext), `resolveCredentials` (§3.4 rule: user binding first with admin fallback when `userManaged`, admin-only otherwise; admin bindings use the all-zeros `ADMIN_CREDENTIAL_USER_ID` sentinel), `hasCredentials` (existence check without decryption), `markCredentialStatus` (verify outcomes), plus pure primitives `encryptCredentialValues`/`decryptCredentialValues` and `maskSecretValues` (exact-value scrubbing for skill output redaction). Plaintext values live only behind a 5-minute in-memory TTL cache invalidated on write. Contract fields added to existing collections: `skills` gains `requiresCredentials`/`userManaged`/`credentialSchema`/`apiDefinitions`/`credentialBinding`/`source`; new `mcpservers` schema/model (`McpServerDoc`) exposes the same credential contract fields plus `credentialBinding` (header mapping for MCP connections).
- Strict three-tier token accounting across messages, model calls, and turn/session totals:
  - **Single message size**: every user/assistant/toolResult message now carries `tokenCount` — the estimated token count of that message's own content only (chars/4 heuristic via `estimateTokens`), set at `message_end` before JSONL/MongoDB persistence. It is never overwritten with call usage or cumulative values.
  - **Per-model-call usage**: each assistant message's actual LLM call usage (`usage.input/output/cacheRead/cacheWrite`) is persisted as separate top-level MongoDB `messages` fields: `inputTokens`, `outputTokens`, `cacheReadTokens`, `cacheWriteTokens`. The legacy `tokenCount = usage.output` conflation was removed; `tokenCount` now always means message own size, and `inputTokenCount` remains as the legacy arp-compatible input field.
  - **Turn/session cumulative usage**: new `UsageTotals` type (`totalInputTokens`, `totalOutputTokens`, `totalCacheReadTokens`, `totalCacheWriteTokens`, each accumulating exactly one category). `AgentSession.getTurnUsage()` returns usage since the last user-initiated message (steering/follow-up injections continue the turn), `SessionStats.usage` exposes session totals, and the MongoDB `conversations` document persists the running session totals on every save. The turn's assistant `messages` document also carries the same four `total*` fields as turn-cumulative usage (all model calls between two user messages; updated on every same-turn merge). `AggregatedUsage` (HTTP SSE `usage` event) now includes the four `total*` counters alongside the OpenAI-style fields.
- `POST /prompt` non-streaming responses now include a `usage` object (per-call breakdown plus turn `total*` counters), and `/v1/chat/completions` usage includes `cache_read_tokens` / `cache_write_tokens`.
- Aligned the legacy `inputTokenCount` field with arp semantics: it now records the model call's full prompt INCLUDING cache (`input + cacheRead + cacheWrite`), matching how arp/LibreChat writes and consumes it for context accounting. Previously pi wrote the non-cached input only, so on shared pi-endpoint documents arp overwrote it with a different caliber. The strict-caliber per-call value remains in `inputTokens`; message reconstruction from arp-written docs now subtracts `cacheReadTokens`/`cacheWriteTokens` from `inputTokenCount` to recover the fresh input.
- Aligned the legacy `tokenCount` field with arp semantics and unified the conversation totals caliber: assistant `tokenCount` now records the turn's cumulative output tokens (was the message's own-size estimate — that value remains authoritative in `agentMessage.tokenCount`); `inputTokenCount` stays at the turn's FIRST call full prompt across same-turn merges (matching arp's firstUsage base); conversations `total*` are recomputed on each save as the sum of every assistant document's turn totals — the same "actual consumption" caliber (all executed turns, regenerated branches included) arp accumulates incrementally.
- Scoped the per-call quartet (`inputTokens`/`outputTokens`/`cacheReadTokens`/`cacheWriteTokens`) on the turn's assistant document to the turn's FIRST model call (stable across same-turn merges), pairing with `inputTokenCount` so tooltip display is a coherent single-call split (`input = cache + miss`); mid-turn call growth only affects the `total*` turn-cumulative fields.
- Fixed the SSE `usage` event and JSON `usage` on `POST /prompt`: the OpenAI-style fields (`prompt_tokens`, `cache_read_tokens`, `cache_write_tokens`, `total_tokens`) are now scoped to the **first** model call of the turn instead of the sum over all internal tool-loop calls (each re-sends the full context, inflating the sum by the number of calls). arp/LibreChat consumes `prompt_tokens` as a single call's prompt and subtracts context messages from it — with the cumulative sum, the second user message's `tokenCount` was ~N× too large (e.g. 198880 vs ~44111 actual). `completion_tokens` stays turn-cumulative (matching arp's native-agent output accounting) and the `total*` fields remain turn-cumulative.
- User-context system-prompt appending migrated from arp (LibreChat). On every HTTP API chat turn (`/prompt` non-skill-execution and `/v1/chat/completions`), pi now appends two per-user sections to the system prompt, read live from MongoDB (skipped when a section is empty or the access check denies): `<available_prompts>` listing the pi-flagged system prompts (`systemprompts` collection, `piPrompt: true` with a valid `piSavePath`) the user has ACL VIEW permission on (`aclentries`, `resourceType: systemPrompt`), each with its server-local file location readable via the read tool; and the `[用户长期记忆]` long-term memory block from the `memoryentries` collection, gated by the arp role permission model (roles grant `MEMORIES.USE`+`MEMORIES.READ`, OR-merged across `users.role`/`users.roles` plus `userroles.roleNames`) and the `users.personalization.memories` opt-out. New mongo services: `prompt-service.ts` (`getAccessiblePiPrompts`, `formatAvailablePromptsPrompt`), `memory-service.ts` (`getUserMemoriesWithAccess`, `formatMemoriesPrompt`, `readMemoryDetail`, `readConversationByMemory`), plus `systemprompts` / `memoryentries` / `users` schemas, models, and doc types.
- `read_memory_detail` and `read_memory_conversation` agent tools (migrated from arp's `/api/memories/details` and `/api/memories/conversation-by-memory` endpoints, previously consumed via skill wrapping). Injected into HTTP API sessions (`/prompt`, `/v1/chat/completions`, `/upload` file processing) only when the user has memory access and at least one memory entry. Both tools read MongoDB directly and are scoped to the session user (`memoryentries.userId`), so cross-user memory reads return "Memory not found".
- Conversation history persistence to MongoDB with bidirectional arp/LibreChat compatibility. Pi stores session message history (user messages, assistant responses, tool results) in the `conversations` and `messages` MongoDB collections, shared with arp. The `conversationId` uses the API-provided `sessionId`. The `conversations` collection includes a new `cwd` field. **MongoDB is the primary history source** — when enabled, pi always loads from MongoDB (clearing any JSONL data to avoid duplicates). Each message document stores both arp-compatible fields (`text`, `content`, `sender`, `tokenCount`, etc.) and a full `agentMessage` field for pi context reconstruction. Messages written by arp (without `agentMessage`) are reverse-constructed from arp fields (`isCreatedByUser`, `text`, `content` array) so pi can continue conversations started in LibreChat. **Tool calls are fully bidirectional**: pi's `ToolCall` content blocks are written as arp `{type:'tool_call', tool_call:{id,name,args}}` content parts, and pi's `ToolResultMessage` updates the matching `tool_call.output` inline (arp format). Arp messages with inline tool calls are split back into pi's `AssistantMessage` + `ToolResultMessage` on load. JSONL remains as a local write-through backup when MongoDB is disabled.
- `CreateAgentSessionOptions.conversationPersistence` for SDK consumers to enable MongoDB-backed conversation history. Pass `{ userId, agentId, conversationId, cwd }` to persist messages to the `conversations` and `messages` collections.
...
- Agent-principal skill authorization on model-side paths. When the request's `agentId` starts with `agent_` (arp agent principal, e.g. `agent_s917T8qpLYVrXzDxIpu4j`), skills are authorized against the agent document's `skills` field (MongoDB `agents` collection) instead of the user ACL. The visibility filter (`createHttpResourceLoader`) loads only the agent's assigned skills, and the runtime path guard (`createSkillPathGuard`) checks `checkAgentSkillPermission` — a missing agent document or a skill not in the agent's list denies access with a "skill does not exist or no permission" error that instructs the model not to retry via other methods. User principals (any other `agentId`) keep the existing ACL-based checks.
...
- HTTP API server startup connects to MongoDB non-fatally; authorized skills are unavailable on connection failure but the server still starts.
  ...
  - `POST /prompt` - Prompt agent with optional streaming support (`stream: true` returns SSE events for thinking, text, tool calls, and final result)
  - API key authentication via `api-key` header (configured via `PI_API_KEY` env var)
  - Configurable port via `--http-port` flag (default: 3000)

### Added

- Per-user sandbox root for the HTTP API. File tools (read/write/edit/bash/grep/find/ls) now accept an `allowedRoot`; in HTTP API sessions it is set to the user's directory `~/.pi/agent/sessions/<userId>/`. An agent can now query a user's own files across all their sessions and create skills under `~/.pi/agent/sessions/<userId>/skills/`, while its working directory stays at the single session directory. `cd` remains restricted to the session cwd so script outputs never leave it.
- `CreateAgentSessionOptions.allowedRoot` and `CreateAgentSessionOptions.bashToolOptions` for SDK consumers to configure the sandbox root and bash tool (spawnHook, sandbox, operations) without passing throwaway `tools` instances.

### Fixed

- `allowedRoot` / `bashToolOptions` were silently dropped by `createAgentSession`. The SDK only used `options.tools` to extract tool names, then `_buildRuntime` rebuilt tools from scratch via `createAllToolDefinitions(cwd, { read, bash: { commandPrefix } })`, ignoring any `allowedRoot` or bash hook/sandbox. These options now flow through `AgentSessionConfig` into `_buildRuntime`, so HTTP API sessions actually get the per-user sandbox root and DMP bash spawnHook.
- Skill output files landing in the skill directory instead of the session working directory. Two layers of defense now keep skill scripts running with the session working directory as their cwd:
  - The bash sandbox now blocks `cd` into a directory outside the working directory (even on the allowed-read list such as `/app/skill-repo`), returning an error that tells the model to invoke the script by absolute path instead. Previously `cd /app/skill-repo/.../skill && python scripts/main.py` was allowed because the target was treated as a readable path, which silently changed the script's cwd and caused its outputs to be written into the skill directory.
  - The skills section of the system prompt now distinguishes reading referenced files (resolve against the skill directory) from running skill scripts (invoke by absolute path, never `cd`), so outputs land in the session working directory.
- Read-only directories (`/app/skill-repo`, `~/.pi/agent/skills`, `~/.pi/agent/prompts`) could be written to or deleted from via the bash sandbox, because `validateCommandSandbox` treated them as freely accessible. The sandbox now classifies write operations — output redirects (`>`, `>>`), and commands such as `rm`, `mv`, `cp` (destination), `mkdir`, `touch`, `chmod`, `chown`, `tee`, `sed -i`, `dd of=` — and rejects them when their target resolves inside a read-only directory, while still allowing those directories to be read and their scripts executed by absolute path.
- Symlink escape from the session sandbox. `resolveToCwdStrict` (used by the write/edit tools) only checked the lexical path, so a symlink placed inside the working directory but pointing outside it bypassed the boundary. It now resolves the real path of the target (or its parent directory for not-yet-existing files) and rejects any target whose real location falls outside the allowed area.
- User prompts directory was unreadable when the process did not run as `root`. `ALLOWED_READ_PREFIXES` hard-coded `/root/.pi/agent/prompts/` and `/home/codeuser/.pi/agent/skills/`; the HTTP API registered the skills directory dynamically but not the prompts directory, and the hard-coded paths were wrong for any other user. The hard-coded list is now empty and the HTTP API registers `/app/skill-repo`, `~/.pi/agent/skills`, and `~/.pi/agent/prompts` dynamically at startup via `addAllowedReadPrefix`.
- Duplicate message documents forking the LibreChat conversation tree when the user stops a generation early. Aborting a turn could finalize two assistant messages back to back; both were persisted concurrently with the same caller-provided `responseMessageId`, and the racing upserts each inserted a document (the shared `messages` collection had no unique index). The tree then showed multiple assistant branches under one user message. Fixes:
  - MongoDB persistence is now serialized per session via a promise chain, so message_end handlers cannot race upserts.
  - Caller-provided `userMessageId` / `responseMessageId` (`setMessageIds`) are single-use per turn; later same-turn messages get fresh ids instead of re-inserting.
  - Same-turn assistant messages always merge into the turn's assistant document (turn state is no longer cleared on non-`toolUse` stop reasons), and the merged document refreshes its `error` flag.
  - The `messages` collection now declares a unique `{ messageId, user }` index (same as arp's schema declaration). `saveMessageToMongo` retries once on duplicate-key errors so concurrent writers converge on one document, and HTTP server startup builds the index via `ensureMessageIndexes()` with a clear log when pre-existing duplicates block it (remove duplicates and restart to let it build).
