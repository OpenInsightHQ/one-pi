...
- Standalone credential references. `skills`/`mcpservers` documents gain `credentialRef` naming a standalone credential (`skillcredentials` docs with `resourceType: "credential"`, created in dmp's credential management). New `resolveCredentialsWithRef`/`hasCredentialsWithRef` resolve own binding first, then the referenced credential (same user/admin fallback rule); used by the catalog status markers and all three `skill_execute` executors (http/mcp/script). `CredentialResourceType` and the mongoose enum accept `"credential"`; `SkillCredentialDoc` gains a plaintext `schemaJson` field (bind-form rendering for standalone entries, values stay encrypted).
